    ...
    async execute(interaction, { client, emojis }) {
        ...
        if (accion === 'set') {
            ...
            try {
                ...
                return interaction.editReply({
                    components: [ui.exito(
                        emojis,
                        `Stock board configured in <#${canal.id}>.\n-# The same message will now update automatically after stock changes and SellAuth invoice events.${mensaje?.url ? `\n[Open stock board](${mensaje.url})` : ''}`
                    )],
                    flags: ui.V2
                });
            } catch (error) {
                return interaction.editReply({
                    components: [ui.error(emojis, `The channel was saved, but the board could not be published: ${ui.plano(error.message)}`)],
                    flags: ui.V2
                });
            }
        }
        ...
        if (accion === 'refresh') {
            try {
                ...
                return interaction.editReply({
                    components: [ui.exito(emojis, `Stock board refreshed successfully.\n-# [Open stock board](${mensaje.url})`)],
                    flags: ui.V2
                });
            } catch (error) {
                return interaction.editReply({
        ...
        const container = new ContainerBuilder()
            .addTextDisplayComponents(new TextDisplayBuilder().setContent(
                ...
                `> Persistent SellAuth inventory image with automatic message updates.`
            ))
